...
export const vueComponentStructure = {
  meta: {
    type: "layout",
    docs: {
      description: "Enforces regional comments and ordering for Vue components, excluding pages. Reorders and creates missing regions with opening and closing tags.",
      category: "Style",
      recommended: true
    },
    fixable: "code",
    messages: {
      structureInvalid: "Component structure is invalid. Preferred order: {{expected}}"
    }
  },

  create(context) {
    if (!context.filename.endsWith(".vue")) return {}
    if (!context.filename.replace(/\\/g, "/").includes("/components/")) return {}
    ...
    return {
      Program(node) {
        ...
        const expectedRegions = ["Props", "Emits", "Slots", "Styles", "State", "Meta", "Lifecycle", "Logic"];
          ...
          context.report({
            ...
            fix(fixer) {
              ...
              expectedRegions.forEach((r, idx) => {
                ...
                result += `/* region ${r} */\n`;
                if (contentsByRegion[key]) {
                  if (contentsByRegion[key].content) {
                    result += contentsByRegion[key].content + '\n';
                  }
                  delete contentsByRegion[key];
                }
                result += `/* endregion */\n`;
                if (idx < expectedRegions.length - 1) {
                  result += '\n';
                }
              });
